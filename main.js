function initialize() {
    ...
    customBtn.addEventListener("click", () => {
        if (hiddenContainer.style.display === "none") { // Showing the hidden div
            if (h2.textContent != "") { // The timer is standing in the way
    ...
    function displayRemainingTime() {
        ...
        if ((currentHours != 0 && currentMinutes === 0 && currentSeconds === 0) || (currentHours === 0 && currentMinutes === 0 && (currentSeconds === 30 || currentSeconds === 10 || currentSeconds < 6))) {
            ...
            h2.style.boxShadow = "0px 2px 5px 6px rgba(153,0,0,1)"
        } else {
        ...
        if (currentHours > 0) {
            h2.innerText = `${`${currentHours}`.length === 2 ? currentHours : `0${currentHours}`}:${`${currentMinutes}`.length === 2 ? currentMinutes : `0${currentMinutes}`}:${`${currentSeconds}`.length === 2 ? currentSeconds : `0${currentSeconds}`}`
        } else {
            h2.innerText = `${`${currentMinutes}`.length === 2 ? currentMinutes : `0${currentMinutes}`}:${`${currentSeconds}`.length === 2 ? currentSeconds : `0${currentSeconds}`}`;
        }
        seconds--;
      }
